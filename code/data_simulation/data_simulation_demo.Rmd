...
```{r}
# Unlist angles and side lengths for BACON
...
for (i in seq_along(dataset)) {
  for (j in seq_along(dataset[[i]])) {
    A[counter, ] <- unlist(angles[[i]][j])
    L[counter, ] <- unlist(side_lengths[[i]][j])
    counter <- counter + 1
  }
}
```
...
```{r}
# Clean up variables
rm(angles, side_lengths, a, l, dataset, shape)
```
...
z <- c(rep(1, z1_length), rep(2, z2_length), rep(3, z3_length), rep(4, z4_length))
```
