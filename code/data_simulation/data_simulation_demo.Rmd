...
performance.
```{r echo = FALSE}
weight_performance <- as.data.frame(matrix(nrow = 20, ncol = 20, byrow = TRUE))
colnames(weight_performance) <- seq(from = 0, to = 2, by = 0.1)
rownames(weight_performance) <- seq(from = 0, to = 2, by = 0.1)
ground_truth <- rep(1:z, each = n)
  
a <- 0 # Weight on angle proportion parameter
l <- 0 # Weight on length proportion parameter
row <- 1 # Row counter

for (i in seq(from = 0, to = 2, by = 0.1)) {
  res <- bacon(side_lengths, 
               angles, 
               K = z,
               weight_L = w,
               ...
               iter = 10000, 
               burn = 5000)
  weight_performance[row, "weight"] <- i
  weight_performance[row, "ari"] <- mclust::adjustedRandIndex(res$cluster, 
                                                         ground_truth)
  a <- a + 0.1
  l <- l + 0.1
