...
// [[Rcpp::export]]
Rcpp::List BACONmcmc(arma::mat L, arma::mat A, int K, IntegerVector z, int iter, int burn) {
  ...
  double map_z, map_z_temp, s_new, map_s, map_s_temp, lambda_temp, theta_temp, lambda_new, theta_new, hastings;
  ...
  for (t = 0; t < iter; t++) {
    ...
    for (j = 0; j < m; j++){
      ...
      for (k = 0; k < K; k++){
        ...
        loglklh(k) = loglklh(k) + lgamma(sum(Lambda(k, _)));
        for (i = 0; i < n; i++) {
          loglklh(k) = loglklh(k) - lgamma(Lambda(k, i)) + (Lambda(k, i) - 1)*log(L2(j, i));
        }
      ...
      for (j = 0; j < m; j++) {
        ...
        for (g = 0; g < 2; g++) {
          for (i = 0; i< n; i++){
            ...
            // Data likelihood of L2
            for (ii = 0; ii< n; ii++) {
              loglklh(i, g) = loglklh(i, g) + (Lambda(z(j), ii) - 1)*log(L2_new(ii));
            }
    ...
    // Calculate posterior probability of s
      ...
      for (i = 0; i < n; i++){
        ...
        // Data likelihood of L2
        for (ii = 0; ii < n; ii++) {
          loglklh(i) = loglklh(i) + (Lambda(z(j), ii) - 1)*log(L2_new(ii));
        }
    ...
    for (j = 0; j < m; j++) {
      ...
      for (g = 0; g < 2; g++) {
        ...
        // Data likelihood of L2
        for (ii = 0; ii < n; ii++) {
          loglklh(g) = loglklh(g) + (Lambda(z(j), ii) - 1)*log(L2_new(ii));
        }
    ...
    // Calculate the marginal posterior probability of s to obtain MAP 
    map_s = 0;
    for (j = 0; j < m; j++) {
      // Prior
    ...
    } // End of calculate MAP of s 
    
    // RWMH to update the concentration parameter Lambda of L
    for (k = 0; k < K; k++){
      for (i = 0; i < n; i++){
        ...
        // Data (L) likelihood ratio
        for (j = 0; j < m; j++){
          if (z(j) == k){
            hastings = hastings + lgamma(sum(Lambda_new(k, _)));
            hastings = hastings - lgamma(sum(Lambda(k, _)));

            for (ii = 0; ii < n; ii++) {
              hastings = hastings + (- lgamma(Lambda_new(k, ii)) + (Lambda_new(k, ii) - 1)*log(L2(j, ii)));
              hastings = hastings - (- lgamma(Lambda(k, ii)) + (Lambda(k, ii) - 1)*log(L2(j, ii)));
            }
          }
        }
    ...
    } // End of updating the concentration parameter Lambda of L


    // RWMH to update the concentration parameter Theta of A
    if (double_dirichlet){
      for (k = 0; k < K; k++){
        for (i = 0; i < n; i++){
    ...
    } // End of updating the concentration parameter Theta of A
  ...
  return Rcpp::List::create(Rcpp::Named("z_store") = z_store, Rcpp::Named("map_z_store") = map_z_store, Rcpp::Named("z_map") = z_map,
                            ...
                            Rcpp::Named("r_store") = r_store, Rcpp::Named("r_ppi") = r_ppi, Rcpp::Named("r_sum") = r_sum,
